...
import type { DailySalesData, SalesHourlyPoint } from "@/mobile/types";
import { getDailySales } from "@/mobile/lib/api";
import { useFetchData } from "@/mobile/hooks/useFetchData";

// 정현파 기반의 얕은 지글지글 노이즈 — 동일 인덱스/씨드는 동일 결과.
function wobble(seed: number): number {
  const s = Math.sin(seed * 12.9898) * 43758.5453;
  return (s - Math.floor(s)) * 2 - 1; // -1..1
}

function densifyChart(points: SalesHourlyPoint[], stepsPerHour = 6): Array<SalesHourlyPoint & { isHour: boolean }> {
  if (points.length < 2) return points.map((p) => ({ ...p, isHour: true }));
  const out: Array<SalesHourlyPoint & { isHour: boolean }> = [];
  for (let i = 0; i < points.length - 1; i++) {
    const a = points[i];
    const b = points[i + 1];
    for (let s = 0; s < stepsPerHour; s++) {
      const t = s / stepsPerHour;
      const baseMy = a.myStore + (b.myStore - a.myStore) * t;
      const baseNb = a.nearby + (b.nearby - a.nearby) * t;
      const ampMy = Math.max(8000, Math.abs(b.myStore - a.myStore) * 0.35);
      const ampNb = Math.max(8000, Math.abs(b.nearby - a.nearby) * 0.35);
      const jitterMy = t === 0 ? 0 : wobble(i * 97 + s * 13 + 1) * ampMy;
      const jitterNb = t === 0 ? 0 : wobble(i * 53 + s * 7 + 31) * ampNb;
      out.push({
        time: s === 0 ? a.time : "",
        myStore: Math.max(0, Math.round(baseMy + jitterMy)),
        nearby: Math.max(0, Math.round(baseNb + jitterNb)),
        isHour: s === 0,
      });
    }
  }
  const last = points[points.length - 1];
  out.push({ ...last, isHour: true });
  return out;
}
...
function CustomTooltip({ active, payload }: CustomTooltipProps) {
  ...
  const myStoreEntry = payload.find((p) => p.name === "내 매장") ?? payload[0];
  return (
    <div className="bg-white border border-[#E5E5EF] rounded-[8px] px-[8px] py-[3px] shadow-[0_2px_6px_rgba(31,151,211,0.1)] whitespace-nowrap">
      <span className="text-[#1E1B39] text-[10px] leading-[10px]">
        ₩ {myStoreEntry.value.toLocaleString("ko-KR")}
      </span>
    </div>
  );
}
...
export default function DailySalesCard() {
  ...
  const { data } = useFetchData(
    ...
    { cacheKey: `getDailySales_${dateKey}`, ttlMs: 30 * 1000 },
    [dateKey],
  );

  const denseChart = useMemo(
    () => (data ? densifyChart(data.chartData, 6) : []),
    [data],
  );
  ...
  return (
    <div className="bg-white border border-[#ebebeb] rounded-[20px] pb-[16px]">
      ...
      {/* 차트 */}
      <div className="bg-[#fff] mx-[20px] rounded-[20px] pt-[4px] pb-[10px]">
            ...
            >
              <defs>
                <linearGradient id="myStoreGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#3aaedd" stopOpacity={0.35} />
              ...
              <Area
                ...
                dot={false}
                activeDot={{ r: 3, fill: "#1F97D3", stroke: "#fff", strokeWidth: 1.5 }}
              />
              {showNearby && (
                <Line
                  type="linear"
                  ...
                  strokeWidth={1.5}
                  dot={false}
                  activeDot={{ r: 3, fill: "#ff522c", stroke: "#fff", strokeWidth: 1.5 }}
                />
              )}
            </AreaChart>
          </ResponsiveContainer>
      ...
      {/* 날짜 선택 */}
      <div className="mx-[20px] bg-[#f0f1f3] rounded-[20px] h-[32px] flex items-center justify-between px-[16px] mt-[10px]">
        <div className="flex items-center gap-[8px]">
          <button
            onClick={() => shiftDate(-7)}
            className="rotate-180 cursor-pointer flex items-center"
            aria-label="이전 주"
          >
            <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
              <path
                d="M1 1L5 5.5L1 10 M5 1L9 5.5L5 10"
                stroke="#666"
                strokeWidth="1.5"
                strokeLinecap="round"
                fill="none"

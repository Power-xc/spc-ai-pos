...
async def analytics_payment_methods(
    ...
    NOTE: payment mix gold table이 있으면 최신 영업일 기준으로 집계합니다.
    """
    from app.tools import sql_queries

    sid = _validate_store_id(store_id or _store_id(request))
    payment_mix = await sql_queries.get_payment_method_mix(db, sid)
    ...
    if methods:
        return APIResponse(
            data={
                ...
                "note": "최신 영업일 결제수단 집계입니다. code_count는 거래 건수 기준입니다.",

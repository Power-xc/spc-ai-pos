...
function formatBackendAnswer(raw: string): string {
  ...
  const actions: unknown[] = Array.isArray(parsed.actions) || Array.isArray(parsed.recommendations)
    ? (Array.isArray(parsed.actions) ? parsed.actions : (parsed.recommendations as unknown[]))
    : [];
...
async function buildSalesReasonResponse(message: string): Promise<LocalMenuResponse | null> {
  try {
    const demoLabel = getDemoDateTimeLabel();
    const salesSummary = await requestJson<{
...
async function buildDashboardResponse(message: string): Promise<LocalMenuResponse> {
  ...
  const displayRevenue = hasCumulative
    ? (salesSummary?.cumulative_revenue_until ?? 0)
    : (salesSummary?.today_revenue ?? 0);
...
async function buildNetSalesExplanation(message: string): Promise<LocalMenuResponse> {
  ...
  const profitabilityRaw = salesSummary?.profitability as
    | Record<string, unknown>
    | undefined;
  const marginRaw = profitabilityRaw?.gross_profit_margin_pct;
  const marginPct =
    marginRaw != null && Number(marginRaw) > 0 ? Number(marginRaw) / 100 : 0.68;
  const netSales = Math.round(displayRevenue * marginPct);
